...
// the token.
bool ReadInt(chromium::JSONReader::Token& token, bool can_have_leading_zeros) {
...
bool ReadHexDigits(chromium::JSONReader::Token& token, int digits) {
...
//  }
  ...
  if (json.size() >= 3 && static_cast<uint8_t>(start_pos_[0]) == 0xEF &&
      static_cast<uint8_t>(start_pos_[1]) == 0xBB &&
      static_cast<uint8_t>(start_pos_[2]) == 0xBF) {
    start_pos_ += 3;
  }
  ...
  std::unique_ptr<Value> root(BuildValue(check_root));
  if (root.get()) {
    ...
    case Token::OBJECT_BEGIN:
      {
        ...
        while (token.type != Token::OBJECT_END) {
          ...
          std::unique_ptr<Value> dict_key_value(DecodeString(token));
...
Value* JSONReader::DecodeNumber(const Token& token) {
  ...
  double num_double;
  if (StringToDouble(num_string, &num_double) && chromium::IsFinite(num_double))
